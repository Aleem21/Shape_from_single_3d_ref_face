...
system (['dcraw-9.26-ms-64-bit -4 -T -W -w -q 3 -h -o 1 ' impath]);
rgb_path = [impath(1:end-3) 'tiff'];
lin_rgb = im2double(imread(rgb_path));
...
% lin_rgb = [];
% lin_gray = [];
% system(['dcraw-9.26-ms-64-bit -6 -T -W -w -q 3 -o 1 ' impath]);
% rgb = im2double(imread(tiff_path));
