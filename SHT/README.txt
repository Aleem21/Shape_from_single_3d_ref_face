...
- Added functions for full rotation of coefficients specified for both real 
  and complex SH

Update 20/2/2015:

- Replaced test_scripts with a single one TEST_SCRIPTS_SHT
- New test_script for plotting functions TEST_SPHPLOT
- Added Fliege-Maier points sets fliegeMaierNodes_1_30.mat
- Added function getFliegeNodes() for returning a specific Fliege-Maier set
- Combined leastSquaresSHT() & weightedLeastSquaresSHT() to a single 
  function leastSquaresSHT()
- Removed tDesignSHT(), replaced with directSHT()
- Added function getVoronoiWeights()
- Updated real2complexCoeffs() and complex2realCoeffs()
- Renamed checkOrthogonality() to checkCondNumberSHT()
- Added plotSphFunctionGrid(), plotSphFunctionTriangle(), plotSphFunctionCoeffs()
